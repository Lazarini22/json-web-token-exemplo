...
export const middleware = async (request) => {
  ...
  if (isTokenValidated || token) {
    //fazendo a validação do token do usuário logado
    if (request.nextUrl.pathname === "/") {
      //verifica se o usuário que possui o token está na página padrão
  ...
  if (!isTokenValidated || !token) {
    //fazendo a validação se o usuário não possui o token -> última  requisição do trabalho
    if (request.nextUrl.pathname === "/pages/register") {
      //verifica se o usuário está na página de "registro" ou na página "alter" -> última  requisição do trabalho
